...
def google_search(search_text):
    ...
    if "who is" or "who are" in search_data:
        search_data = search_data.split(" ")[2:]
        search_data = " ".join(search_data)
        try:
            result = wikipedia.summary(search_data, sentence=3)
        ...
        try:
            ...
            result_div = soup.find_all('div', class_='BNea we s3v9rd AP7Wnd')

            if "born" in search_data:
                for i in result_div:
                    s = translator.translate(dest='fr', text=i.text)
